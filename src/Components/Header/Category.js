            ...
            { selectToggle && 
                <ul>
                    ...
                    {categories && categories.map((item)=>{
                        return <li onClick={selectCategoryHandler} key={item.id}>{item.title}</li>
                    })}
                    <li className={styles.addLi} onClick={()=>dispatch(todoActions.categoryPortalToggle())}>
                            Add Category  <FontAwesomeIcon icon={faPlus} className={styles.addIcon} />
                    </li>
                </ul>}
        </div>
    )
