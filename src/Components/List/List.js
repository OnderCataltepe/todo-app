...
const List = () =>{
    ...
    if(isLoading){
        return <div className={styles.loadErrorContainer}> <Loading /></div>
    }
    if(error){
        return <div className={styles.loadErrorContainer}> <Error errorMessage={error} /></div>
    }
        ...
        {todoList.length===0 &&  
            <div>
                ...
                {selCategory.title !=="All" && isAll && 
                        <button className={styles.removeCategory} onClick={removeCategoryHandler}>
                            Remove Category
                        </button>
                }
            </div>
